...
export function LogForm({ handleOpenLog, open }) {
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault()
      setLogin(false)
      const email = e.target.phoneNumber.value
      ...
      >
        <Card className="mx-auto w-full max-w-[24rem]">
          <CardBody className="flex flex-col gap-2">
            ...
            <Input label="Номер телефона" name="phoneNumber" size="lg" defaultValue={'+998'} />

            {/*<p className="text-sm"><span className="text-red-700">*</span>Номер телефона без знака '+'</p>*/}

            <Input label="Пароль" name="password" size="lg" />
          ...
          <CardFooter className="pt-0">
            <Button variant="gradient" onClick={handleOpenLog} type="submit" fullWidth>
